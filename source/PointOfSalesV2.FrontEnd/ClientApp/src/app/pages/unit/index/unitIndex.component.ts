...
export class UnitIndexComponent extends BaseComponent implements OnInit {
    ngOnInit(): void {
        this.verifyUser();
        this.getPagedData(1);
    }
...
this.filterData(e.value,config.id,config.objectType,config.isTranslated);
  ...
  filterIsActive:temp.filterIsActive
    ...
    addNew() {
        this.router.navigateByUrl(`pages/unit/add`);
    }
    edit(e:Unit) {
        this.router.navigateByUrl(`pages/unit/edit/${e.id}`);
    }
    source:any={};
    onDeleteConfirm(event:Unit): void {
