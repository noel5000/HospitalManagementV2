...
export class InvoicePaymentFormComponent extends BaseComponent implements OnInit {
  ...
  constructor(@Inject('BASE_URL') private baseUrl: string,
    ...
    this.paymentService= new BaseService<any, number>(this.http, `${this.baseUrl}api/CustomerPayment`);
    this._route = router;
  ...
  ngOnInit(): void {
    ...
    const urlId = parseInt(this._route.snapshot.paramMap.get('invoiceId') ?? '0');
    if (!isNaN(urlId)) {
  ...
  override onChanges(): void {
    ...
    this.itemForm.get('paymentTypeId')?.valueChanges.subscribe(val => {
      ...
      if (val && val > 0) {
        const paymentType = this.paymentTypes.find((x:any) => x.id == val);
  ...
  save() {
    ...
    const toPost = {
      ...
      invoices: this.invoices.filter((x:any) => x.currentPaidAmount > 0),
      userId: this.authModel?.user?.userId
    };
    const subscription = this.paymentService.post(toPost, '', 'PayInvoices');
  ...
  refreshTotalAmount(index: number, hasValue: boolean) {
    ...
    const paidSumm = this.invoices.reduce(function (a:any, b:any) { return a + b.currentPaidAmount }, 0);
