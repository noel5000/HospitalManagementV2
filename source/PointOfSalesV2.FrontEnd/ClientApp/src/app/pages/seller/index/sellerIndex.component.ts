...
export class SellerIndexComponent extends BaseComponent implements OnInit {
    ngOnInit(): void {
        this.verifyUser();
        this.getPagedData(1);
    }
...
this.filterData(e.value,config.fieldToShow?config.fieldToShow: config.id,config.objectTypeToShow?config.objectTypeToShow: config.objectType,config.isTranslated);
  ...
  filterIsActive:temp.filterIsActive
    ...
    addNew() {
        this.router.navigateByUrl(`pages/seller/add`);
    }
    edit(e:Seller) {
        this.router.navigateByUrl(`pages/seller/edit/${e.id}`);
    }
    source:any={};
    onDeleteConfirm(event:Seller): void {
