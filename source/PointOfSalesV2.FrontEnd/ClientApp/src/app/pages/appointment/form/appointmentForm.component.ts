...
export class appointmentFormComponent extends BaseComponent implements OnInit {
  ...
  CalculateProductTax(): number {
    ...
    return this.productTaxes.length <= 0 ? 0 : this.productTaxes.reduce(function (a:any, b:any) { return a + (b.tax.rate * productPrice) }, 0);
  }

  async GetProductCost(productId: number) {
    const currentProduct = this.products.find((x:any) => x.id == productId);
    this.currentProductCost.cost = currentProduct!.cost;
    const { unitId } = this.itemForm.getRawValue();
    this.currentProductPrice.sellingPrice = unitId && unitId > 0 && this.productUnits.length > 0 ? this.productUnits.find((x:any) => x.unitId == unitId).sellingPrice : currentProduct!.price;
    this.refreshAmounts();
  }
  ...
  async getHospitals() {
    this.branchOfficeService.getAll().subscribe(r => {
      this.hospitals = [({ id: null, name: "" } as any) as BranchOffice];
...
selectedPatient:any=null;
  ...
  override onChanges(): void {
    ...
    this.itemForm.get('productId')!.valueChanges.subscribe(val => {
      if (val && val > 0) {
        const product = this.products.find((x:any) => x.id == val);
        this.itemForm.patchValue({
          insuranceCoverageAmount: 0,
  ...
  async addDetail() {
    ...
    const detail = {
      ...
      medicalSpecialityId:form.medicalSpecialityId=="null"?null:form.medicalSpecialityId,
      product: form.productId ? this.products.find((x:any) => x.id == form.productId) : null,
      doctor: form.doctorId ? this.doctors.find((x:any) => x.userId == form.doctorId) : null,
      typeObj: form.type ? this.types.find((x:any) => x.id == form.type) : null,
      medicalSpeciality:form.medicalSpecialityId && form.medicalSpecialityId!="null"?this.medicalSpecialities.find(x=>x.id==form.medicalSpecialityId):null,
  ...
  refreshAmounts(fromForm: boolean = false) {
    ...
    const equivalence = unitId && unitId > 0 ? this.productUnits.find((x:any) => x.unitId == unitId).equivalence : 1;
