...
export class InventoryEntryFormComponent extends BaseComponent implements OnInit {
  ...
  inventoryEntryService: BaseService<any, number> ;
  ...
  constructor(@Inject('BASE_URL') private baseUrl: string,
    ...
    super(route, langService, AppRoles.Movements_InventoryEntry, modalService);
    this.inventoryEntryService = new BaseService<any, number>(this.http, `${this.baseUrl}api/InventoryEntry`);
  ...
  async getWarehouses(id: number) {
    ...
    this.warehouseService.getAllFiltered(filter).subscribe(r => {
      ...
      if (data.length && data.length > 0) {
        data = data.filter((x:any) => x.code != 'DEF')
      }
