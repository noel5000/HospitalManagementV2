...
export class patientCheckupEditFormComponent extends BaseComponent implements OnInit {
  ...
  medicines: Product[] = [];
  attachmentsService: BaseService<any, number> ;
  ...
  constructor(@Inject('BASE_URL') private baseUrl: string,
    ...
    this.attachmentsService= new BaseService<any, number>(this.http, `${this.baseUrl}api/CheckupAttachment`);
    this.service= new BaseService<any, number>(this.http, `${this.baseUrl}api/PatientCheckUp`);
    ...
    this._route = router;
    const urlId = parseInt(this._route.snapshot.paramMap.get('checkupid') ?? '0');
    const appointmentId = parseInt(this._route.snapshot.paramMap.get('appointmentid') ?? '0');
    const patientId = parseInt(this._route.snapshot.paramMap.get('patientid') ?? '0');
    const editing = parseInt(this._route.snapshot.paramMap.get('editing') ?? '0');
  ...
  async labTestSelection(index: number) {
    ...
    if (labTest.selected) {
      ...
      let prescription = {
        ...
        type: 'L',
        product: labTest,
        medicalSpeciality: this.medicalSpecialities.find((x:any) => x.id == medicalSpecialityId)
      };
      if (selectedIndex < 0) {
  ...
  async getAppointment(id: number) {
    this.appointmentService.getById(id).subscribe(r => {
      ...
      if (!this.appointment)
        ...
        const consultationDetail = this.appointment.details.find((x:any) => x.type == 'C');
  ...
  async getProductsByNameAndType(name: string, type: string) {
    if (name) {
      const filter = [
        {
          property: "Currency",
  ...
  async getItem(id: number) {
    this.service.getById(id).subscribe(r => {
      try {
        if (r.status >= 0) {
          this.item = r.data[0];
          this.selectedMedicines = this.item.checkupPrescriptions.filter((x:any) => x.type == "M");
          this.selectedLabTests = this.item.checkupPrescriptions.filter((x:any) => x.type == "L");
          this.selectedConsultations = this.item.checkupPrescriptions.filter((x:any) => x.type == "C");
          this.selectedImages = this.item.checkupPrescriptions.filter((x:any) => x.type == "E");
  ...
  async addPrescription(type: string) {
    ...
    var quantityByWhenToTake = whenToTake ? whenToTake.toString().split('-').filter((x:any) => x == '1') : [];
    let prescription = {
      productId,
      ...
      medicalSpeciality: this.medicalSpecialities.find((x:any) => x.id == this.selectedProduct.medicalSpecialityId)
    };
  ...
  findProduct(id: number, type: string): Product {
    ...
    switch (type) {
      case 'L':
        result = this.labTests.find((x:any) => x.id == id);
        break;
      case 'M':
        result = this.medicines.find((x:any) => x.id == id);
        break;
      case 'E':
        result = this.specilizedimages.find((x:any) => x.id == id);
        break;
      case 'C':
        result = this.medicalSpecialities.find((x:any) => x.id == id);
        break;
    }
    return result;
  }
