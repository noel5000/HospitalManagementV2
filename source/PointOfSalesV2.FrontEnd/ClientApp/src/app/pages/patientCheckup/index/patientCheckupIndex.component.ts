    ...
    this.service.getById(id).subscribe(r=>{
        try{
            if(r.status>=0){
                ...
                const selectedMedicines=checkup.checkupPrescriptions.filter((x:any)=>x.type=="M");
                const selectedLabTests=checkup.checkupPrescriptions.filter((x:any)=>x.type=="L");
                const selectedConsultations=checkup.checkupPrescriptions.filter((x:any)=>x.type=="C");
                const selectedImages=checkup.checkupPrescriptions.filter((x:any)=>x.type=="E");
