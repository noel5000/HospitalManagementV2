...
export class InvoiceFormComponent extends BaseComponent implements OnInit {
  ...
  constructor(@Inject('BASE_URL') private baseUrl: string,
    ...
    this.hospitalId = this.authModel?.user?.branchOfficeId ?? 0;
    this.warehouseId = this.authModel?.user?.warehouseId ?? 0;
  ...
  async getWarehouses() {
    ...
    this.warehouseService.getAllFiltered(filter).subscribe(r => {
      ...
      this.warehouses = this.warehouses.concat((r['value'] as Warehouse[]).filter((x:any) => x.code != 'DEF'));
    });
  }
  ...
  async GetDetailProductTaxes(index: number) {
    ...
    this.productTaxService.getAllFiltered(filter).subscribe(r => {
      ...
      entry.taxesAmount = taxes.length <= 0 ? 0 : taxes.reduce(function (a:any, b:any) { return a + (b.tax.rate * (entry.beforeTaxesAmount - entry.discountAmount)) }, 0);
  ...
  override onChanges(): void {
    ...
    this.itemForm.get('productId')?.valueChanges.subscribe(val => {
      if (val && val > 0) {
        ...
        this.productPrices = [{ id: 1, price: product.price, currency: product.currency.code }, { id: 2, price: product.price2, currency: product.currency.code }, { id: 3, price: product.price3, currency: product.currency.code }].filter((x:any) => x.price > 0);
    ...
    this.appointmentService.getById(id).subscribe(r => {
      ...
      if (!this.isEditing) {
        ...
        if (this.selectedAppointment.details && this.selectedAppointment.details.length > 0) {
          this.selectedAppointment.details.forEach((d:any, i:number) => {
            const detail = {
              quantity: 1,
              doctorId: d.doctorId,
  ...
  addEntry() {
    ...
    const rate = !patientCurrency ? 0 : patientCurrency.isLocalCurrency ? (this.selectedProductCurrency ? this.selectedProductCurrency.exchangeRate : 1) :
      (this.selectedProductCurrency?.exchangeRate ?? 0 / patientCurrency.exchangeRate);
