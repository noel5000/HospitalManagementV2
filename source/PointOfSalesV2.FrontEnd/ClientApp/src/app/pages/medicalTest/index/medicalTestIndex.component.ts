...
export class MedicalTestIndexComponent extends BaseComponent implements OnInit {
    ngOnInit(): void {
        this.verifyUser();
        this.getPagedData(1);
    }
...
this.actions=[
    ...
    getData() {
        ...
        this.filters.push({property:'Type', value:'L,E', comparer:ODataComparers.in, type: ObjectTypes.String  } as QueryFilter)
...
this.filterData(e.value,config.fieldToShow?config.fieldToShow: config.id,config.objectTypeToShow?config.objectTypeToShow: config.objectType,config.isTranslated);
  ...
  filterIsActive:temp.filterIsActive
    ...
    addNew() {
        this.router.navigateByUrl(`pages/medical-test/add`);
    }
    edit(e:Product) {
        this.router.navigateByUrl(`pages/medical-test/edit/${e.id}`);
    }
    source:any={};
