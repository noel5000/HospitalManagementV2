...
@Component({
  standalone:false,
    ...
    templateUrl: "./warehouseTransfersIndex.component.html",
    styleUrls: ["../warehouseTransfersStyles.component.scss"]
})
export class WarehouseTransferIndexComponent extends BaseComponent implements OnInit {
    ngOnInit(): void {
        this.verifyUser();
        this.getPagedData(1);
    }
    ...
    constructor(@Inject('BASE_URL') private baseUrl: string,
       ...
       modalService:ModalService
        ...
        this.service = new BaseService<any,number>(this.http, `${this.baseUrl}api/WarehouseTransfer`);
        let scope = this;
...
this.actions=[
    ...
    getData() {
        this.service.getFiltered(this.pageNumber, this.pageSize, this.filters, this.orderBy, this.orderDirection).subscribe(r => {
            ...
            this.WarehouseTransfers=r['value'];
...
this.filterData(e.value,config.fieldToShow?config.fieldToShow: config.id,config.objectTypeToShow?config.objectTypeToShow: config.objectType,config.isTranslated);
  ...
  filterIsActive:temp.filterIsActive
    ...
    addNew() {
        this.router.navigateByUrl(`pages/warehousetransfer/add`);
    }
    source:any={};
