...
export class QuotesFormComponent extends BaseComponent implements OnInit {
  ...
  inventoryService: BaseService<any, number> ;
  creditNoteService: BaseService<any, number> ;
  invoiceService: BaseService<any, number> ;
  menuService: BaseService<any, number> ;
  currencyService: BaseService<any, number> ;
  productUnitService: BaseService<any, number> ;
  productTaxService: BaseService<any, number> ;
  medicalSpecialityService: BaseService<any, number> ;
  appointmentService: BaseService<any, number> ;
  userService: BaseService<any, number> ;
  insuranceService: BaseService<any, number>;
  appointments: any[] = [];
  selectedAppointment: any = null;
  oldProductCost: number = 0;
  oldProductPrice: number = 0;
  selectedProductCurrency: Currency | null = null;
  ...
  constructor(@Inject('BASE_URL') private baseUrl: string,
    ...
    this.inventoryService= new BaseService<any, number>(this.http, `${this.baseUrl}api/Inventory`);
    this.creditNoteService= new BaseService<any, number>(this.http, `${this.baseUrl}api/CreditNote`);
    this.invoiceService= new BaseService<any, number>(this.http, `${this.baseUrl}api/Invoice`);
    this.menuService= new BaseService<any, number>(this.http, `${this.baseUrl}api/Menu`);
    this.currencyService= new BaseService<any, number>(this.http, `${this.baseUrl}api/Currency`);
    this. productUnitService= new BaseService<any, number>(this.http, `${this.baseUrl}api/ProductUnit`);
    this.productTaxService= new BaseService<any, number>(this.http, `${this.baseUrl}api/ProductTax`);
    this.medicalSpecialityService= new BaseService<any, number>(this.http, `${this.baseUrl}api/MedicalSpeciality`);
    this.appointmentService= new BaseService<any, number>(this.http, `${this.baseUrl}api/Appointment`);
    this.userService= new BaseService<any, number>(this.http, `${this.baseUrl}api/User`);
    this.insuranceService= new BaseService<any, number>(this.http, `${this.baseUrl}api/InsuranceServiceCoverage`);
    this.verifyUser();
  ...
  override setAdditionalBackupData() {
  ...
  async getWarehouses() {
    ...
    this.warehouseService.getAllFiltered(filter).subscribe(r => {
      ...
      this.warehouses = this.warehouses.concat((r['value'].sort(this.dynamicSort('name')) as Warehouse[]).filter((x:any) => x.code != 'DEF'));
    });
  }

  async getSellers(patientId: number) {
    const patient = this.patients.find((x:any) => x.id == patientId);
    this.itemForm.patchValue({
  ...
  async GetDetailProductTaxes(index: number) {
    ...
    this.productTaxService.getAllFiltered(filter).subscribe(r => {
      ...
      entry.taxesAmount = taxes.length <= 0 ? 0 : taxes.reduce(function (a: number, b: { tax: { rate: number; }; }) { return a + (b.tax.rate * (entry.beforeTaxesAmount - entry.discountAmount)) }, 0);
  ...
  CalculateProductTax(): number {
    ...
    return this.productTaxes.length <= 0 ? 0 : this.productTaxes.reduce(function (a:any, b:any) { return a + (b.tax.rate * productPrice - (b.tax.rate * productPrice * discountRate / 100)) }, 0);
  }
  ...
  async GetProductCost(productId: number) {
    ...
    this.currentProductPrice.sellingPrice = unitId && unitId > 0 && this.productUnits.length > 0 ? currentProduct.price / this.productUnits.find((x:any) => x.unitId == unitId).equivalence : currentProduct.price;
    this.refreshAmounts();
  }
  ...
  override onChanges(): void {
    ...
    this.itemForm.get('unitId')?.valueChanges.subscribe(val => {
      ...
      if (val && val != "0") {
        this.refreshAmounts();
        const currentUnit = this.productUnits.find((x:any) => x.unitId == val);
    ...
    this.itemForm.get('patientId')?.valueChanges.subscribe(val => {
      ...
      if (val && this.patients && this.patients.length > 0) {
        ...
        const patient = this.patients.find((x:any) => x.id == val);
        if (patient) {
          this.itemForm.patchValue({
            trnControlId: patient.trnControlId,
            insuranceId: patient.insuranceId,
    ...
    this.itemForm.get('productId')?.valueChanges.subscribe(val => {
      if (val && val > 0) {
        ...
        this.productPrices = [{ id: 1, price: product.price, currency: product.currency.code },
          { id: 2, price: product.price2, currency: product.currency.code }, { id: 3, price: product.price3, currency: product.currency.code }].filter((x:any) => x.price > 0);
        ...
        if (product.isService) {
          ...
          this.getProductInventory(user!.branchOfficeId ?? 0, form.warehouseId == null ? 0 : form.warehouseId, val)
  ...
  async getPatientAppointments() {
    ...
    if (patientId && patientId > 0) {
      ...
      this.appointmentService.getAllFiltered(filter).subscribe(r => {
        ...
        r['value'].forEach((a: { [x: string]: string; date: string; }) => {
    ...
    this.appointmentService.getById(id).subscribe(r => {
      ...
      if (!this.isEditing) {
        ...
        if (this.selectedAppointment.details && this.selectedAppointment.details.length > 0) {
          this.selectedAppointment.details.forEach((d:any, i:any) => {
            const detail = {
              quantity: 1,
              doctorId: d.doctorId,
  ...
  updateSelectedPrice(val:number) {
    const { productId, unitId } = this.itemForm.getRawValue();
    if (productId && productId > 0) {
      let price = val;
      if (unitId) {
        const productUnit = this.productUnits.find((x:any) => x.unitId == unitId);
  ...
  async selectProduct(product: any) {
    if (product) {
      ...
      if (product.type == 'C')
        this.getDoctors(product.medicalSpecialityId, this.getUser()!.branchOfficeId ?? 0);
    }
    else {
      this.product = null;
      this.itemForm.patchValue({
  ...
  addEntry() {
    ...
    entry.unit = entry.unitId ? this.productUnits.find((x:any) => x.unitId == entry.unitId).unit :null;
    entry.id = 0;
    ...
    entry.taxesAmount = entry.product.taxes.length <= 0 ? 0 : entry.product.taxes.reduce(function (a:any, b:any) { return a + (b.tax.rate * (entry.beforeTaxesAmount - entry.discountAmount)) }, 0);
  ...
  validateCheck(validation: any) {
    let validateCheck = document.getElementById(`noCoverage_${validation.index}`) as any;
  ...
  setDetailFormNoCoverage(index: number, noCoverage: boolean = false) {
    if (!this.itemForm.contains(`noCoverage_${index}`))
      this.itemForm.addControl(`noCoverage_${index}`, new FormControl(noCoverage , []));
  }
  ...
  refreshAmounts(fromForm: boolean = false) {
    ...
    if (productId && productId > 0 && this.patient && this.patient.id) {
      const equivalence = unitId && unitId > 0 && this.productUnits ? this.productUnits.find((x:any) => x.unitId == unitId).equivalence : 1;
      const patientCurrency = this.patient.currency;
      const rate = !patientCurrency ? 0 : patientCurrency.isLocalCurrency ? (this.selectedProductCurrency ? this.selectedProductCurrency.exchangeRate : 1) :
        (this.selectedProductCurrency!.exchangeRate / patientCurrency.exchangeRate);
