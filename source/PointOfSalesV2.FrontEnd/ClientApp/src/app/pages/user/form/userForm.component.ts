...
export class UserFormComponent extends BaseComponent implements OnInit {
  ...
  languages: any[] = [];
  languagesService: BaseService<any, number> ;
  medicalSpecService: BaseService<any, number>;
  genders: any[] = [
    {
  ...
  constructor(@Inject('BASE_URL') private baseUrl: string,
    ...
    this.itemForm = this.formBuilder.group({
      ...
      email: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(150), Validators.email]],
    ...
    this.languagesService = new BaseService<any, number>(this.http, `${this.baseUrl}api/language`);
    this.medicalSpecService = new BaseService<any, number>(this.http, `${this.baseUrl}api/MedicalSpeciality`);
  }
  ngOnInit(): void {
    this.onChanges();
  ...
  getBranchOfficeWarehouses(branchOfficeId: number) {
    this.warehouseService.getAllFiltered(
      ...
      this.warehouses = [({ name: '', id: null } as any) as Warehouse];
      this.warehouses = this.warehouses.concat((x['value'].sort(this.dynamicSort('name')) as Warehouse[]).filter((x:any) => x.code != 'DEF'));

    })
  }
  async getBranchOffices() {
    this.branchOfficeService.getAll().subscribe(r => {
      this.branchOffices = [({ id: null, name: '' } as any) as BranchOffice];
