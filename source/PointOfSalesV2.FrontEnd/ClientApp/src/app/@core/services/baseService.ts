...
export class BaseService<TEntity, TKey> implements IService<TEntity, TKey> {
  ...
  _headers: HttpHeaders | null = null;
  httpOptions = {};
  public get headers() {
  ...
  getODataQuery(filters: QueryFilter[], page: number, max: number, orderBy: string, direction: string): string {
    let result = '';
    const expandFilters = filters.filter((x:any) => x.type == ObjectTypes.ChildObject);
    ...
    let query = '$filter=';
    const filterData = filters.filter((x:any) => x.type != ObjectTypes.ChildObject);
  ...
  public setLanguageInHeaders(languageId: string) {
    if (languageId) {
      ...
      let headers = languageId
        ...
        : tempHeader;
      let tempHttpOptions = this.tempHttpOptions as any;
      let currentOptions = this.httpOptions as any;
      tempHttpOptions['headers'] = headers;
      tempHttpOptions['params'] = currentOptions['params'];
      this.tempHttpOptions = tempHttpOptions;
    }
  }
  ...
  SetTranslationData(data: any): any {
