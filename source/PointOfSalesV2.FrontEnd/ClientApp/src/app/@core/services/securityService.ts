    ...
    isUserValidInMenu(role: AppRoles): boolean {
        ...
        if (auth && new Date(auth.expiration) > new Date()) {
            if (auth.user.permissions.length == 0)
                return true;
            if (auth.user.permissions.filter((x:any) => x.roleId == role && (x.operationId == Operations.READ || x.operationId == Operations.ALL)).length > 0)
                return true;
        }
        return result;
    }
    ...
    validateMenuChildren(roles: AppRoles[]): boolean {
        ...
        if (auth && new Date(auth.expiration) > new Date()) {
            if (auth.user.permissions.length == 0)
                return true;
                roles.forEach(s => {
                if (auth.user.permissions.filter((x:any) => x.roleId == s && (x.operationId == Operations.READ || x.operationId == Operations.ALL)).length > 0)
                    result= true;
            })
        }
        return result;
